...
func newTestService(t *testing.T) (*Service, *fakeFiles) {
	t.Helper()
	files := &fakeFiles{}
	svc := NewService(Deps{DB: newTestDB(t), JWTSecret: testJWTSecret, Files: files})
	return svc, files
}
...
func TestLogin_Success(t *testing.T) {
	...
	token, user, err := svc.Login("alice@example.com", "password123", "", "", 0)
	if err != nil {
...
func TestLogin_WrongCredentials(t *testing.T) {
	...
	if _, _, err := svc.Login("alice@example.com", "wrong", "", "", 0); !errors.Is(err, ErrInvalidCredentials) {
		t.Errorf("expected ErrInvalidCredentials, got %v", err)
	}
	if _, _, err := svc.Login("nobody@example.com", "password123", "", "", 0); !errors.Is(err, ErrInvalidCredentials) {
...
func TestLogin_EmailUnverified(t *testing.T) {
	svc, _ := newTestService(t)
	enableSMTP(t, svc.db)
	seedUser(t, svc.db, "alice@example.com", "password123", model.RoleAuthor, false)

	if _, _, err := svc.Login("alice@example.com", "password123", "", "", 0); !errors.Is(err, ErrEmailUnverified) {
...
func TestLogin_WithCaptcha(t *testing.T) {
	...
	if err := svc.db.Create(&captcha).Error; err != nil {
		t.Fatalf("failed to seed captcha: %v", err)
	}
	if _, _, err := svc.Login("alice@example.com", "password123", "c1", "t1", 10); !errors.Is(err, ErrCaptchaMismatch) {
		t.Errorf("expected ErrCaptchaMismatch, got %v", err)
	}

	// correct position passes
	token, _, err := svc.Login("alice@example.com", "password123", "c1", "t1", 100)
	if err != nil {
	...
	if err := svc.db.First(&stored, "id = ?", "c1").Error; err != nil {
...
func TestRegister_Success(t *testing.T) {
	...
	result, err := svc.Register("new@example.com", "password123", "newbie", "", "", 0, "http", "localhost:8080")
	if err != nil {
	...
	var stored model.User
	if err := svc.db.First(&stored, result.User.ID).Error; err != nil {
		t.Fatalf("failed to reload user: %v", err)
	}
...
func TestRegister_DuplicateEmail(t *testing.T) {
	svc, _ := newTestService(t)
	seedUser(t, svc.db, "dup@example.com", "password123", model.RoleGuest, true)

	if _, err := svc.Register("dup@example.com", "password123", "other", "", "", 0, "http", "localhost"); !errors.Is(err, ErrUserExists) {
...
func TestChangePassword(t *testing.T) {
	svc, _ := newTestService(t)
	u := seedUser(t, svc.db, "alice@example.com", "oldpass", model.RoleGuest, true)

	// wrong old password
	if err := svc.ChangePassword(u.ID, "nope", "newpass123"); !errors.Is(err, ErrWrongPassword) {
		t.Errorf("expected ErrWrongPassword, got %v", err)
	}

	// success: version incremented and new password works
	if err := svc.ChangePassword(u.ID, "oldpass", "newpass123"); err != nil {
		t.Fatalf("ChangePassword error: %v", err)
	}
	var stored model.User
	if err := svc.db.First(&stored, u.ID).Error; err != nil {
		t.Fatalf("failed to reload user: %v", err)
	}
	if stored.PasswordVersion != u.PasswordVersion+1 {
		t.Errorf("expected password version incremented")
	}
	if _, _, err := svc.Login("alice@example.com", "newpass123", "", "", 0); err != nil {
		t.Errorf("expected new password to work, got %v", err)
	}
	if _, _, err := svc.Login("alice@example.com", "oldpass", "", "", 0); !errors.Is(err, ErrInvalidCredentials) {
...
func TestUpdateSettings(t *testing.T) {
	svc, _ := newTestService(t)
	u := seedUser(t, svc.db, "alice@example.com", "password123", model.RoleGuest, true)

	hideEmail := true
	visibility := "private"
	user, err := svc.UpdateSettings(u.ID, UpdateSettingsRequest{
	...
	if _, err := svc.UpdateSettings(99999, UpdateSettingsRequest{}); !errors.Is(err, ErrUserNotFound) {
...
func TestUpdateProfile_DeletesOldAvatar(t *testing.T) {
	...
	user, err := svc.UpdateProfile(u.ID, UpdateProfileRequest{Avatar: &newAvatar})
	if err != nil {
...
func TestResetPassword(t *testing.T) {
	...
	// invalid token
	if err := svc.ResetPassword("nope", "newpass123"); !errors.Is(err, ErrInvalidResetToken) {
	...
	if err := svc.ResetPassword("expired-token", "newpass123"); !errors.Is(err, ErrResetTokenExpired) {
		t.Errorf("expected ErrResetTokenExpired, got %v", err)
	}

	// success: token cleared
	if err := svc.ResetPassword("reset-token", "newpass123"); err != nil {
